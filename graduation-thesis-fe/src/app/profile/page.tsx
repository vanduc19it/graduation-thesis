...
import { FaFacebook, FaInstagram, FaLinkedin, FaRegHeart, FaTwitter } from "react-icons/fa";
...
const Profile = () => {
        ...
        if (typeof window !== "undefined") {
          ...
          if (nft && marketplace && accounts) {
            ...
            try {
              ...
              //listing nft:
              const myNftListing = await marketplace.getListingsByOwner(accounts[0]);
              ...
              //purchased nft
              const myNftPurchased = await marketplace.getPurchasedItems(accounts[0]);
              ...
              localStorage.setItem("nftPurchased", JSON.stringify(PurchasedResults));
  ...
  useEffect(() => {
    ...
    const nftCreatedData: any = nftCreatedLocal ? JSON.parse(nftCreatedLocal) : [];
    setNftCreated(nftCreatedData);

    const nftListingLocal: any = localStorage.getItem("nftListing");
    const nftListingData: any = nftListingLocal ? JSON.parse(nftListingLocal) : [];
    setNftListing(nftListingData);

    const nftPurchasedLocal: any = localStorage.getItem("nftPurchased");
    const nftPurchasedData: any =  nftPurchasedLocal ? JSON.parse( nftPurchasedLocal) : [];
  ...
  const handleListForSale = async (item: any) => {
    ...
    const handleGetUser = async(addressWallet:any) => {
      try {
        const response = await axios.get(`http://localhost:5000/get_user/${addressWallet}`
        );
...
const handleFileChange = async(e:any) => {
  ...
  if (file) {
    try {
      ...
      if(data) {
        try {
          const response = await axios.put(`http://localhost:5000/update_avatar/${address}`, {
...
const handleFileChangeCover = async(e:any) => {
  ...
  if (file) {
    try {
      ...
      if(data) {
        try {
          const response = await axios.put(`http://localhost:5000/update_cover_photo/${address}`, {
...
const { isOpen: isOpenModalName, onOpen: onOpenModalName, onClose: onCloseModalName } = useDisclosure();
const [userName, setUserName] = useState("");

const handleUpdateUsername = async () => {
  try {
    const response = await axios.put(`http://localhost:5000/update_username/${address}`, {
    ...
    <>
      ...
      <div>
        <div style={{ margin: "0 -70px", position:"relative" }} >
          <div className={styles["cover-container"]}  style={{ width: "100%", height: "300px" }}>
            {loadingImage1 ? (
            ...
            src={(user == "" || user?.cover_photo=="") ? "/image/cover.jpg":user.cover_photo }
            alt="background"
            loading="lazy"
            style={{ backgroundImage: "cover", width: "100%", height: "300px" }}
          />
          <div  className={styles["edit-icon-cover"]} onClick={() => inputRef1.current!.click()}>
             <MdOutlineEdit />
          </div>
          ...
          <div style={{display:"flex", alignItems:"center",width:"80%",height: "230px",boxShadow: "0 4px 8px rgba(0, 0, 0, 0.1)", position:"absolute", marginTop:"-100px", background:"white", borderRadius:"30px"}}>
          <div
            style={{
              display: "flex",
          ...
          > {
            ...
            />) : (
              ...
              <Image
              className={styles.nft_img}
              src={(user == "" || user?.avatar == "") ? "/image/avatar-default-icon.png": user.avatar }
              alt="avatar"
              loading="lazy"
              style={{ borderRadius: "20px", width: "150px", height: "150px" }}
            />
             <div  className={styles["edit-icon"]} onClick={() => inputRef.current!.click()}>
             <MdOutlineEdit />
              </div>
        ...
        <Text
          style={{ fontSize: "24px", fontWeight: "600", marginBottom: "4px", marginLeft: 20 }}
        >
          {user !== "" ? user?.name : "Unnamed"}
          
        </Text>
        <div onClick={onOpenModalName}>
        <MdOutlineEdit style={{fontSize:"22", marginBottom: "4px", cursor:"pointer"}} />
        </div>
        
        </div>

        <div style={{display:"flex", gap:8, alignItems:"center", position:"absolute", right:40}}>
          <Button style={{borderRadius:"20px", background:"#ae4cff", color:"white"}}>Edit</Button>
          <span style={{display:"flex",width:"36px", height:"36px",borderRadius:"50px",background:"#ddd", alignItems:"center", justifyContent:"center"}}><BiUpload /></span>

          <span  style={{display:"flex",width:"36px", height:"36px",borderRadius:"50px",background:"#ddd", alignItems:"center", justifyContent:"center"}}><AiOutlineEllipsis /></span>
        </div>
        </div>
            ...
            address !== "" ? (
              <>
               <Image
                ...
                />
                <Text style={{ color: "#484848", marginRight: "14px", marginLeft:"2px" }}>

                  {address?.substring(0, 6) + "..." + address?.substring(38)}
                </Text>
                <Text style={{ color: "#6d6d6d" }}>{formatJoinedDate(user?.created_at)}</Text>
              </>
            ) : (<></>)
          }
         
        </div>
        <div style={{marginLeft: "20px", width:"500px", fontSize:"14px", fontStyle:"italic", marginTop:"5px"}}>
          <Text>I make art with the simple goal of giving you something pleasing to look at for a few seconds.</Text>
        </div>
        <div style={{display:"flex", alignItems:"center", gap: 8, marginLeft:"20px", marginTop:"5px", color:"#ae4cff"}}>
        <FaFacebook />
        <FaTwitter />
        <FaLinkedin />
        <FaInstagram />
        ...
        <Tabs style={{border:"white", marginTop:"180px"}}>
          <TabList style={{ marginTop: "22px" }}>
            <Tab style={{ fontWeight: "500", fontSize: "16px", background:"#ae4cff", borderRadius:"30px", color:"white", marginRight:"10px", boxShadow: "0 4px 8px 2px rgba(0, 0, 0, 0.1)" }}>Created {nftCreated.length}</Tab>
            <Tab style={{ fontWeight: "500", fontSize: "16px", background:"#fff", borderRadius:"30px", color:"black", marginRight:"10px" , boxShadow: "0 4px 8px 2px rgba(0, 0, 0, 0.1)"}} >Listing {nftListing.length}</Tab>
            <Tab style={{ fontWeight: "500", fontSize: "16px", background:"#fff", borderRadius:"30px", color:"black", marginRight:"10px" , boxShadow: "0 4px 8px 2px rgba(0, 0, 0, 0.1)"}}>Purchased {nftPurchased.length}</Tab>
            <Tab style={{ fontWeight: "500", fontSize: "16px", background:"#fff", borderRadius:"30px", color:"black", marginRight:"10px" , boxShadow: "0 4px 8px 2px rgba(0, 0, 0, 0.1)"}}>
              Live Auctions {nftAuction.length}
            </Tab>
            <Tab style={{ fontWeight: "500", fontSize: "16px", background:"#fff", borderRadius:"30px", color:"black", marginRight:"10px" , boxShadow: "0 4px 8px 2px rgba(0, 0, 0, 0.1)"}}>Favorited {nftLiked.length}</Tab>
          </TabList>
                  ...
                  nftCreated.map((item: any, id) => (
                    ...
                    >
                      ...
                      <div className={styles.price}>
                        <Button
                          style={{ background: "#ae4cff", color: "white", borderRadius:"20px" }}
                        ...
                        <Button
                          style={{ background: "#ea0061", color: "white", borderRadius:'20px' }}
                ...
                {nftAuction.length > 0 &&
                  nftAuction.map((item: any, id) => (
                    ...
                    >
                      ...
                      <div className={styles.button_buy}>
                        <Button
                          style={{ border: "1px solid black", color: "#000", borderRadius: "20px",  }}
                          variant="outline"
                          isDisabled
